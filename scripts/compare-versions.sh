...
# 检查必需文件
if [ ! -f "$PROJECT_ROOT/backend/Dockerfile.vllm" ]; then
    echo -e "${RED}错误: Dockerfile.vllm 不存在${NC}"
    exit 1
fi
...
test_version() {
    ...
    # 备份当前Dockerfile
    cp "$PROJECT_ROOT/backend/Dockerfile.vllm" "$PROJECT_ROOT/backend/Dockerfile.vllm.tmp"
    
    # 使用指定版本
    if [ -f "$DOCKERFILE" ]; then
        cp "$DOCKERFILE" "$PROJECT_ROOT/backend/Dockerfile.vllm"
    else
        echo -e "${YELLOW}警告: $DOCKERFILE 不存在，使用当前版本${NC}"
    fi
    
    echo -e "${YELLOW}停止现有容器...${NC}"
    docker compose -f "$PROJECT_ROOT/docker-compose.vllm.yml" down
    
    echo -e "${YELLOW}构建镜像...${NC}"
    docker compose -f "$PROJECT_ROOT/docker-compose.vllm.yml" build --no-cache
    
    echo -e "${YELLOW}启动容器...${NC}"
    docker compose -f "$PROJECT_ROOT/docker-compose.vllm.yml" up -d
    ...
    if [ $ELAPSED -ge $MAX_WAIT ]; then
        ...
        docker compose -f "$PROJECT_ROOT/docker-compose.vllm.yml" logs
        return 1
    fi
    ...
    echo ""
    cd "$SCRIPT_DIR"
    ./benchmark-vllm.sh http://localhost:8000 "$TEST_IMAGE" $ITERATIONS 5
    ...
    # 收集容器日志
    docker compose -f "$PROJECT_ROOT/docker-compose.vllm.yml" logs > "$RESULTS_DIR/${VERSION_NAME}_logs.txt"
    
    # 恢复Dockerfile
    mv "$PROJECT_ROOT/backend/Dockerfile.vllm.tmp" "$PROJECT_ROOT/backend/Dockerfile.vllm"
...
# 1. 测试当前版本
echo -e "${BLUE}===========================================${NC}"
test_version "current" "$PROJECT_ROOT/backend/Dockerfile.vllm"

# 2. 测试渐进式更新版本（如果存在）
if [ -f "$PROJECT_ROOT/backend/Dockerfile.vllm.updated" ]; then
    echo -e "${BLUE}===========================================${NC}"
    read -p "是否测试渐进式更新版本？(y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        test_version "updated" "$PROJECT_ROOT/backend/Dockerfile.vllm.updated"
    fi
fi

# 3. 测试最新版本（如果存在）
if [ -f "$PROJECT_ROOT/backend/Dockerfile.vllm.latest" ]; then
    echo -e "${BLUE}===========================================${NC}"
    read -p "是否测试最新版本？(y/n) " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
        test_version "latest" "$PROJECT_ROOT/backend/Dockerfile.vllm.latest"
    fi
fi
